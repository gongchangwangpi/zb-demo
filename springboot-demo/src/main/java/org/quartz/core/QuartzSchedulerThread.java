...
public class QuartzSchedulerThread extends Thread {
    ...
    boolean sleepLog = true;
    long mainLoopTime = System.currentTimeMillis();
    ...
    public void run() {
        ...
        while (!halted.get()) {
            try {
                ...
                log.info("=====> main loop time = {}, availThreadCount = {}, acquiresFailed = {}", System.currentTimeMillis() - mainLoopTime, availThreadCount, acquiresFailed);
                mainLoopTime = System.currentTimeMillis();
                ...
                if(availThreadCount > 0) { // will always be true, due to semantics of blockForAvailableThreads...
                    ...
                    try {
                        ...
                        log.info("===> 查询到下一次执行的triggers size: {}", triggers.size());
                        acquiresFailed = 0;
                    ...
                    if (triggers != null && !triggers.isEmpty()) {
                        ...
                        for (int i = 0; i < bndles.size(); i++) {
                            ...
                            log.info("===> 开始执行job ： {}", bndle.getJobDetail().getJobClass());
                ...
                // 如果查找到有需要执行的trigger，上面直接continue了，不会到这里
                log.info("=====> 本次循环没有查找到需要执行的trigger，默认wait 23-30秒， timeUntilContinue = {}", timeUntilContinue);
                synchronized(sigLock) {
                    try {
                        if(!halted.get()) {
