...
public final class FTPUtil {
	...
	private FTPClient getFtpClient(String... middleDir) {
		...
		try {
			...
			log.info("----->>> getFtpClient use time: {}ms", end - start);
	...
	private FTPClient connect(FTPClient client) throws IOException {
		...
		log.info("----->>> connect config use time: {}ms", configEnd - loginEnd);
	...
	public boolean uploadFile(String fileName, InputStream input, String... middleDir) {
		...
		try {
			...
			long end = System.currentTimeMillis();
			log.info("----->>> uploadFile use time: {}ms", end - start);
		} catch (Exception e) {
			flag = false;
			log.error("上传文件失败", e);
		} finally {
	...
	public void closeFTPClient() {
		...
		try {
			...
			long end = System.currentTimeMillis();
			log.info("----->>> returnFTPClient use time: {}ms", end - start);
