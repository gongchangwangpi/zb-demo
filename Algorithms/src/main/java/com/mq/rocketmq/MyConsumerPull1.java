 ...
 */
public class MyConsumerPull1 {

    private final Logger logger = LoggerFactory.getLogger(MyConsumerPull1.class);
...
//        defaultMQPullConsumer.setMessageModel(MessageModel.BROADCASTING);
        ...
        Set<MessageQueue> messageQueues = defaultMQPullConsumer.fetchSubscribeMessageQueues("MyTopic1");
        messageQueues.forEach(queue -> {
            PullResult pullResult = null;
            try {
                pullResult = defaultMQPullConsumer.pull(queue, "MyTag1", getOffset(queue), 32);
